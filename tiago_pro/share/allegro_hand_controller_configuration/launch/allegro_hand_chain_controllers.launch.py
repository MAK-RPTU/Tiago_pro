...
def declare_actions(launch_description: LaunchDescription, launch_args: LaunchArguments):
    ...
    launch_controller = generate_load_controller_launch_description(
        ...
        extra_spawner_args=["--activate-as-group"]
        )
...
def setup_controller_configuration(context: LaunchContext):
    ...
    controller_name = f"{ee_prefix}_controller_pids {ee_prefix}_controller"
    remappings = {"EE_SIDE_PREFIX": ee_prefix}
    param_file = os.path.join(
        get_package_share_directory('allegro_hand_controller_configuration'),
        'config', 'allegro_hand_chain_controller.yaml')
